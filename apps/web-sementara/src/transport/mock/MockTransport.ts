...
import { KREDIT_MACET_SCRIPT } from "./scripts/kreditMacet.script";
import { KEANGGOTAAN_FIKTIF_SCRIPT } from "./scripts/keanggotaanFiktif.script";
import { RAPAT_ANGGOTA_TAHUNAN_SCRIPT } from "./scripts/rapatAnggotaTahunan.script";
...
const SCRIPTS: Partial<Record<ScenarioId, ScenarioScript>> = {
  ...
  "rapat-anggota-tahunan": RAPAT_ANGGOTA_TAHUNAN_SCRIPT,
};
...
/** Emitter minimal: cukup untuk semua stream, tanpa dependency. */
class Emitter<T> {
...
/**
 * Transport terskrip untuk validasi UI tanpa backend.
 *
 * Bagian generik (streaming partial→final, drift keyword, force-quit, pemilihan
 * ending) dipakai semua skenario. RAT (`script.rat`) menambah alur dua-persona
 * + state machine fase di atas fondasi yang sama.
 */
export class MockTransport implements SessionTransport {
  ...
  connect(scenarioId: ScenarioId): Promise<void> {
    ...
    if (!script) {
      ...
      return Promise.reject(
        new Error(
          `MockTransport belum punya skrip untuk skenario "${scenarioId}".`,
        ),
      );
    }
    ...
    return new Promise((resolve) => {
      this.later(() => {
        ...
        // Level awal selalu 0 (netral); untuk skenario tanpa drift ini satu-
        // satunya nilai yang pernah diemit — indikator inert tapi ter-wire.
        this.drift.emit(0);
  ...
  sendText(text: string): void {
    ...
    // driftLevel === 2 berarti force quit sudah berjalan — abaikan input lanjut.
    if (
      !trimmed ||
      this.ended ||
      this.disposed ||
  ...
  advancePhase(): void {
    ...
    // Interupsi terskrip saat masuk fase (mis. Pak Darma di Fase 2).
    if (phase.entryEvent) {
  ...
  debugSetDrift(level: DriftLevel): void {
    ...
    if (config) {
      this.driftScore =
        level === 2 ? config.level2At : level === 1 ? config.level1At : 0;
    }
    this.setLevel(level);
  ...
  private evaluateDrift(playerText: string): DriftLevel {
    ...
    const level: DriftLevel =
      this.driftScore >= config.level2At
        ? 2
        : this.driftScore >= config.level1At
          ? 1
          : 0;
    this.setLevel(level);
    return level;
  ...
  /** NPC menutup diri lalu sesi berakhir otomatis (PRD §6 jalur c). */
  ...
  /** Responder = penyebutan nama → default fase → default global. */
  ...
  // --- streaming & util (generik) ---------------------------------------

  /** Emit ulang dengan id sama + teks bertambah, persis pola partial→final. */
